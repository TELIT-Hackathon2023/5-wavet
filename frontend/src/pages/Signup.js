...
const Signup = () => {
    ...
    const [firstNameCon, setFirstNameCon] = useState(true);
    const [lastNameCon, setLastNameCon] = useState(true);
    ...
    useEffect(() => {
        setFirstNameCon(firstName.length < 3 && firstName.length > 0)
    }, [firstName])

    useEffect(() => {
        setLastNameCon(lastName.length < 3 && lastName.length > 0)
    }, [lastName])
    ...
    return (
        <div className=" mt-16  w-screen flex items-center justify-center flex-col ">
            <form className="w-[90vw] md:w-[35vw] bg-white  border border-borders p-8 child:my-2 mt-16 flex flex-col justify-end" onSubmit={register}>
                <h1 className="font-semibold text-xl">Registrácia</h1>
                ...
                {firstNameCon && <p className="error">Zadaj meno</p>}
                {lastNameCon && <p className="error">Zadaj priezvisko</p>}
